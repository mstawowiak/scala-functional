...
object Huffman {
   ...
   */
  def until[T](isSingleton: List[T] => Boolean, combine: List[T] => List[T])(trees: List[T]): List[T] =
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    ...
    def loop(subTree: CodeTree, bits: List[Bit], acc: List[Char]): List[Char] = subTree match {
      ...
      case Fork(l, r, _, _) => if (bits.head == 0) loop(l, bits.tail, acc) else loop(r, bits.tail, acc)
    }
  ...
  val secretDecoded: List[Char] = List('h', 'u', 'f', 'f', 'm', 'a', 'n', 'e', 's', 't', 'c', 'o', 'o', 'l')
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def loop(t: CodeTree)(ch: Char): List[Bit] = t match {
      case Leaf(_, _) => List()
      case Fork(l, r, _, _) => if (chars(l).contains(ch)) 0 :: loop(l)(ch) else 1 :: loop(r)(ch)
    }

    text flatMap loop(tree)
  }

  def encoded: List[Bit] = encode(frenchCode)(secretDecoded)
  ...
  def mergeCodeTables(a: CodeTable, b: CodeTable): CodeTable = {
    def prepend(b: Bit)(codeTable: CodeTable): CodeTable = codeTable map (code => (code._1, b :: code._2))
