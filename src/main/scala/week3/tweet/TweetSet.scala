...
object GoogleVsApple {
  ...
  def tweetsForCompany(keywords: List[String]): TweetSet =
    TweetReader.allTweets.filter(p => keywords.exists(r => p.text.contains(r)))

  lazy val googleTweets: TweetSet = tweetsForCompany(google)
  lazy val appleTweets: TweetSet = tweetsForCompany(apple)
